...
// The API key is a unique value. It is assigned the GOOGLE_API_KEY which came from library. The user would have had to register for it.
...
app.get('/api/v1/books/find', (req, res) => {
  ...
  // query is being defined as an empty string. If title,author,isb are queried, they will add that to query variable. The title, author and isbn are coming from the object that's passed in to the req. 
  let query = ''
  ...
  // superagent is a json package which is an API for ajax and node. It's authenticating the request. You can find many other libraries available by doing a search for public APIs 
  superagent.get(url)
    .query({'q': query})
    .query({'key': API_KEY})
    .then(response => response.body.items.map((book, idx) => {
      ...
      // This image will display an image when there is no image_url. It's part of the ternary operator below in the return.
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // DONE: Explain how ternary operators are being used below.
      // ternary operators are conditionals. For example, the code below is asking "If there is no title, return 'No title available' in the title place." If there is a title, it will show. 
      return {
...
// ISBN is referring to the findOne function that gets/creates the book by ISBN. This route is different because it finding by isbn rather than finding all. 
